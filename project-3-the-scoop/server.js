...
function createComment(url, request) {
  ...
  if(request.body && request.body.comment){
    ...
    savedUser = database.users[requestComment.username];
    savedArticle = database.articles[requestComment.articleId];
  }
  // Is a valid request?
  if ( !requestComment || !requestComment.body || !requestComment.articleId || !requestComment.username || !savedUser || savedArticle === undefined || request.body === undefined) {
      response.status = 400;
  } else {
    const comment = {
      ...
      // Add comment id to author comment id
      database.users[comment.username].commentIds.push(comment.id);
      // Add comment id to article comment id
...
function updateComment(url, request) {
  ...
  if (!id || !requestComment ) {
    response.status = 400;
  } else if (!savedComment || id !== requestComment.id) {
    response.status = 404;
  } else {
...
function deleteComment(url) {
  ...
  if (savedComment) {
    ...
    let index = database.articles[savedComment.id].commentIds.indexOf(savedComment.id);
    if (index > -1){
      database.articles[savedComment.articleId].commentIds.splice(index,1);
    }
    index = database.users[savedComment.username].commentIds.indexOf(savedComment.id);
    if (index > -1){
      database.users[savedComment.username].commentIds.splice(index,1);
    }
    response.status = 204;
  } else {
    response.status = 404;
...
function upvoteComment(url, request) {
    const id = Number(url.split('/').filter(segment => segment)[1]);
    const requestedUser = request.body && request.body.username;
    const savedUser = database.users[requestedUser];
    const savedComment = database.comments[id];
    const response = {};


    if (savedUser === undefined || savedComment === undefined) {
      response.status = 400;
    } else {

        if(savedComment.upvotedBy.indexOf(requestedUser) === -1){
        database.comments[id].upvotedBy.push(requestedUser);

          if(savedComment.downvotedBy.indexOf(requestedUser) !== -1){
            const index = savedComment.downvotedBy.indexOf(requestedUser);
            database.comments[id].downvotedBy.splice(index, 1);
          }
          response.body = {};
...
function downvoteComment(url, request) {
  ...
  if (savedUser === undefined || savedComment === undefined) {
    response.status = 400;
      ...
      if(savedComment.downvotedBy.indexOf(requestedUser) === -1){
      database.comments[id].downvotedBy.push(requestedUser);

        if(savedComment.upvotedBy.indexOf(requestedUser) !== -1){
          const index = savedComment.upvotedBy.indexOf(requestedUser);
          database.comments[id].upvotedBy.splice(index, 1);
        }
        response.body = {};
        response.body.comment = database.comments[id];
      }
